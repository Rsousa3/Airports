...
int main() {
    ...
    //------------------3 POSIÇÃO------------------------------------------------------
                ...
                if (partidas.size() == 0) {
                    cout << "Não existem aeroportos com as condições dadas.";
                    return 0;
                }
            } else {
                cout << "Input inválido";
    ...
    //------------------------------CHEGADAS------------------------------------------------------
            cout
                    << "Que aeroporto(s) de chegada pretende selecionar?\n 1. Aeroporto específico.\n 2. Aeroportos por cidade.\n 3. Aeroportos por posição.\n";
            cin >> opc;
    //-----------------1 AEROPORTO-----------------------
            if (opc == 1) {
                cout << "Indique o código do aeroporto:\n";
            ...
            if (opc == 1) {
                ...
                while (loop) {
                    ...
                    if (opc == 1) {
                        ...
                        cin >> input;
                        if (!g.checkInput(input, false)) {
                            cout << "Input inválido! Introduza o código novamente.\n";
                            continue;
                        }
                        companhias.insert(input);
                    }
    ...
    //-------------------------INFORMATION----------------------------
            cout
                    << "Selecione o tipo de informação que pretende obter:\n 1. Nº de voos de um aeroporto\n 2. Informações de voos\n";
            cin >> opc;
    //------------------------COUNT VOOS------------------------------------------
            if (opc == 1) {
    ...
    //-------------------------INFO VOOS--------------------------------------
                ...
                cout
                        << "Introduza um número máximo de voos permitidos a partir do aeroporto para obter informação: "; //se calhar mudar phrasing
                cin >> ninput;
    ...
    //------------------------STATS-DE-PAÍS-----------------------------------------
            ...
            cin.ignore();
            getline(cin, input);
            if (!g.checkInputLocal(input, false)) {
                cout << "Input inválido!";
                return 0;
            }
            g.getStats(input);
        ...
        cin >> opc;
        if (opc == 1) { repeat = true; }
        if (opc == 2) { repeat = false; }
